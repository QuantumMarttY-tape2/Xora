...
const Header = () => {
    // The state that helps the header to have a background and smaller after scrolling down from top.
    const [hasScrolled, setHasScrolled] = useState(false);

    // Functionality to toggle menu for mobile devices.
    const [isOpen, setIsOpen] = useState(false);

    useEffect(() => {
        const handleScroll = () => {
            // Confirm scrolling after scroll for 32 px.
            setHasScrolled(window.scrollY > 32);
        }

        // For browser to track this.
        window.addEventListener("scroll", handleScroll);

        // Remove the listener once it is not needed.
        return () => {
    ...
    return (
        <header className={clsx("fixed top-0 left-0 z-50 w-full py-10 transition-all duration-500 max-lg:py-4",
            ...
            <div className="container flex h-14 items-center max-lg:px-5">
                ...
                {/* Button for mobile devices to toggle menu. */}
